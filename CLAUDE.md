...
python -m explorer_agent.evaluate --client <client_id> [--run-id <id>] [--source rules|duplicates|llm|all]
...
### Deterministic SAP rule pack (`explorer_agent/sap_rules.py`)

Known SAP standards are **data, not LLM output**: `explorer_agent/rule_packs/sap_master_data.yaml` (`sap_rules.pack_file`) lists them and `sap_rules.run_sap_rules` executes them per table with **zero LLM calls**, right after duplicate detection and before the planner (so its findings survive an LLM outage, and `--deterministic-only` runs them without any planner/reflector). Families, one finding per rule and table with every offending row (`reusable=False`, `rule_scope=UNIVERSAL`, `check_code` starts with `# Built-in SAP rule <id>`):

- **ACTIVENESS**: status flags set (`LOEVM`/`SPERM`/`SPERR`/`LVORM`/…, severity LOW, since a flag is a scoping fact, not an error); **dormancy** = `ERDAT` older than N years **and** no row in any org-level table (LFB1/LFM1, KNB1/KNVV). `ERDAT` is a creation date, so age alone never means inactive; without transaction data this is the best evidence. Skipped when no child table is loaded.
- **COMPLETENESS**: mandatory fields per table (`any_of` for alternatives such as BANKN/IBAN). All `MANUAL_FIX` by default; a client makes one `AUTO_FIXABLE` only through `sap_rules.client_overrides.<client>.auto_fix`. Only SAP-standard mandatory fields are in the pack (KOINH is not); client requirements go in `client_overrides.<client>.mandatory`.
- **CORRECTNESS**: ISO country keys; postal code format by country; **tax numbers evaluated across all tax fields of a row** (STCD1…STCD5, STCEG). A row is "missing" only when none holds a real value, and each filled value must match one of its country's formats (EU VAT + national). A blank STCD1 is never an error on its own (Indian GSTIN lives in STCD3). Unknown countries are skipped, never failed.
- **RELATIONSHIP_INTEGRITY**: orphan child rows (skipped when the parent table isn't loaded, e.g. `--tables LFB1`), and master-level flags not carried to org-level rows, only where standard SAP sets both levels together (central deletion flag → LFB1/LFM1; not MARA→MARC, not bank data on deleted vendors).

Meaning divergence is handled by configuration, not code: `sap_rules.disabled_rules` / `client_overrides.<client>.disabled_rules` (an id or a prefix like `flag` or `tax.LFA1`), and a `guard` on propagation rules that checks the client's data dictionary (standard SAP: LFA1-SPERM = purchasing block → LFM1; a dictionary that calls it a posting block switches to the LFB1-SPERR rule instead). `run_sap_rules` returns a `RuleCoverage`: its `lines` go into the planner prompt (rule descriptions only, never data), and `graph.py` **drops planner checks whose (column, category[, sub_type]) a rule already covers**, because models ignore the prompt instruction (observed: re-proposed LOEVM/ERDAT/LAND1/AKONT checks, one suggesting to auto-fill AKONT). `metrics.sap_rules_evaluated/sap_rule_findings/sap_rule_rows/planner_checks_covered_by_rules` report this.
...
3. **Deterministic where business meaning isn't required.** Matching, execution and extraction are plain Python, and known SAP standards are a rule pack (`sap_rules.py`), not planner output. The LLM decides only *what a column means* and *what to check beyond the standards*, and those decisions are persisted (duplicate rules, skills) so later runs are free and auditable.
