...
2. **The LLM budget is bounded and independent of column count.** Each table costs 2 calls (planner + reflector), plus 1 duplicate-rule call and, for a non-SAP-standard layout, 1 column-mapping call the first time a client's schema is seen. Cache hits cost less. A feature that adds per-column or per-row LLM calls needs a stated reason and must show up in `metrics.py`.
